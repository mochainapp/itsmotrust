...
const eventFired = (eventName: string, receipt: ContractReceipt) =>
  receipt.events?.find((ev) => ev.event === eventName) ?? undefined;
...
describe("EscrowHub Contract", () => {
  ...
  describe("Methods", () => {
    it("Should Create Escrow & Emit EscrowCreated event", async () => {
      ...
      expect(escrowCreatedEvent?.args?.state).to.equal(
      ...
      // @ts-ignore
      const escrowsAsSeller = await (
        escrowHub.connect(address2) as unknown as EscrowHub
      ).fetchMyEscrows();
      ...
      // @ts-ignore
      const escrowsAsOwner = await (
        escrowHub.connect(owner) as unknown as EscrowHub
      ).fetchMyEscrows();
