...
export const useBalanceQuery = (options = {}) => {
  ...
  return useQuery<BigNumberish>(
    ...
    () => {
      return toastQuery<BigNumberish>(provider.getBalance(signerAddress), {
    ...
    async () => {
      ...
      try {
        ...
        if (metaDataFromLocalStorage) {
          const metaData: EscrowMeta = JSON.parse(metaDataFromLocalStorage);
          ...
          while (
            ...
            try {
              const { data: metaData } = await axios.get<EscrowMeta>(
                sprintf(settings.ipfsGateways[gatewayIndex], cid, "meta.json"),
                {
                  headers: {
              ...
              dataFromRequest.attachments = dataFromRequest.attachments.map(
                (fileName) =>
                  sprintf(settings.ipfsGateways[gatewayIndex], cid, fileName)
              );
...
export const useSingleEscrowQuery = (
  ...
  const { escrowContract } = useContext(EscrowContext);
  return useQuery<EscrowResponse | null>(["singleEscrow", escrowId], () => {
  ...
  return useMutation(
    ...
    (data: NewEscrowInputs) => {
      const storeEscrow = async () => {
        if (!escrowContract) throw new Error("Escrow contract not found");
    ...
    {
      onSuccess(data, variables, context) {
        const event = data?.events?.length ? data?.events[0] : undefined;
        queryClient.invalidateQueries(["escrowPaginated"], {
          refetchType: "all",
        });
        if (event?.args?.length) {
          queryClient.setQueriesData<EscrowMeta>(
            ["escrowMeta", event?.args[1]],
            {
              ...
              attachments: (variables.doc as unknown as Array<File>)?.map(
                (file: File) => file.name
              ),
            }
          );
