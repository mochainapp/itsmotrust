...
export const EscrowItem = ({ escrow }: { escrow: EscrowResponse }) => {
  ...
  const { data: meta, isLoading: metaLoading } = useEscrowMetaQuery(escrow.cid);
  ...
  const meAs = useMemo(() => {
    ...
    switch (address) {
      ...
      case escrow.seller.toLowerCase():
        return "seller";
      case process.env.NEXT_PUBLIC_DEPLOYER_ADDRESS:
        return "owner";
      default:
        return "unknown";
    }
  ...
  const QrcodeImage = useMemo(() => {
    ...
    return (
      <QRCode
        id={`react-qrcode-logo-${escrow.id}`}
  ...
  return (
    <div className={`escrow-card border border-1 border-${contextualClass}`}>
      <span className={`escrow-status badge bg-${contextualClass}`}>
        {statusText}
      </span>
      <h3>
      ...
      <ul className="list-group mt-3">
        ...
        {escrow.state === EscrowState.AWAITING_DELIVERY && meAs === "buyer" && (
