...
export const AllEscrows = () => {
  ...
  const allMyEscrowsFiltered = useMemo(() => {
    ...
    return allMyEscrows?.filter((escrow) => escrow.state === parseInt(type));
  }, [type, allMyEscrows]);
    ...
    <>
      <div className="row g-3 align-items-center justify-content-between mb-4">
        ...
        <div className="col-md right_filter">
          ...
          <Form.Select
            ...
            onInput={(e) =>
              setType((e.target as HTMLInputElement).value ?? "-1")
            }
          >
            <option value={-1}>All</option>
            <option value={EscrowState.AWAITING_DELIVERY}>Pending</option>
          ...
          {!!allMyEscrows?.length && (
            <div className="row">
              {allMyEscrowsFiltered.length ? (
                ...
                <h2 className="text-center mb-3">You have no escrows</h2>
              )}
            </div>
          )}
          {!allMyEscrows?.length ? (
            <div className="my-5">
              ...
              <div className="text-center">
                <NewEscrow className="btn-lg">Create One</NewEscrow>
              </div>
            </div>
          ) : null}
          {hasNextPage && (
      ...
      {searchKey && (
        <>
          ...
          {!searchResultLoading &&
            (!searchResultEscrow || searchResultEscrow?.id?.toNumber() < 1) && (
