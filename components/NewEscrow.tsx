...
export const NewEscrow = ({ children, className }: any) => {
  ...
  const { symbol } = useContext(EscrowContext);
  ...
  useEffect(() => {
    if (web3Loading) {
      window.onbeforeunload = () =>
        "Web3 transaction is running. Do you really force your browser to close?";
    } else {
  ...
  return (
    <>
      <button
        className={`btn new_escrow btn-warning ${className}`}
        onClick={() => setNewEscrowModal(true)}
      >
        {children ?? "New Escrow"}
      </button>

      <Modal show={newEscrowModal} onHide={() => setNewEscrowModal(false)}>
        <Form onSubmit={handleSubmit(onEscrowSubmit)}>
          ...
          <Modal.Body>
            <Form.Group className="mb-3" controlId="contractTitle">
              <Form.Label>
                Escrow Title <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                type="text"
            ...
            <Form.Group className="mb-3" controlId="contractDescription">
              <Form.Label>Escrow Description</Form.Label>
              <Form.Control
                as="textarea"
            ...
            <Form.Group className="mb-3" controlId="sellerAddress">
              <Form.Label>
                Seller Address <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                type="text"
            ...
            <Form.Group className="mb-3" controlId="escrowAmount">
              <Form.Label>
                Escrow Amount <span className="text-danger">*</span>
              </Form.Label>
              <InputGroup>
                <Form.Control
                  type="number"
              ...
              <Form.Text className="text-muted">
                Fee: {cAmount.fee}, Escrow Amount: {cAmount.escrow}
              </Form.Text>
            </Form.Group>

            <Form.Group className="mb-3" controlId="escrowExpire">
              <Form.Label>
                Expires On <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                type="date"
              ...
              />
              <Form.Text className="text-muted">
                Seller can automatically claim the fund after expiration
              </Form.Text>
            </Form.Group>

            <Form.Group className="mb-3" controlId="doc">
              <Form.Label>Upload Detailed Information</Form.Label>
              <Form.Control type="file" multiple {...register("doc")} />
              <Form.Text className="text-muted">
                Can be contract paper, NDA, any pdf, doc file etc
              </Form.Text>
            </Form.Group>
          </Modal.Body>
          <Modal.Footer>
            ...
            <Button variant="primary" className="Submit_button" type="submit">
              Submit
            </Button>
          </Modal.Footer>
        </Form>
